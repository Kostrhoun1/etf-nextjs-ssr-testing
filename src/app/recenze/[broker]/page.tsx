...
export async function generateMetadata(
  ...
  return {
    ...
    alternates: { canonical: `https://www.etfpruvodce.cz/recenze/${b.id}` },
  };
}
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 antialiased">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm text-slate-600">
            ...
            <Link href="/kde-koupit" className="text-teal-700 font-medium">Kde koupit</Link>
          </nav>
          <HeaderSearch />
      ...
      <main className="max-w-6xl mx-auto px-4 pb-14">
        ...
        <nav aria-label="Drobečková navigace" className="py-3 text-xs text-slate-500 flex items-center gap-1.5">
          ...
          <span>/</span>
          <Link href="/srovnani-brokeru" className="hover:text-slate-700">Brokeři</Link>
          <span>/</span>
        ...
        <section className="pb-8">
          <div className="flex flex-wrap gap-3">
            <Link href="/srovnani-brokeru" className="inline-flex items-center gap-2 rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-medium text-white hover:bg-teal-800">Srovnat s dalšími brokery <ArrowRight className="w-4 h-4" /></Link>
