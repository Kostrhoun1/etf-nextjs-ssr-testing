    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 antialiased">
      ...
      <main className="max-w-6xl mx-auto px-4 pb-28">
        ...
        <nav className="py-3 text-xs text-slate-400 flex items-center gap-1.5">
          ...
          <span>/</span>
          <Link href="/zebricky" className="hover:text-slate-600">Nejlepší ETF</Link>
          <span>/</span>
        ...
        <section id="srovnani" className="pb-10 scroll-mt-16">
          <SectionHead title="Srovnání hlavních S&P 500 ETF" desc="Největší a nejvýznamnější fondy sledující čistý index S&P 500. Přepněte měnu výnosů." href="/srovnani" hrefLabel="srovnat vše" />
        ...
        {/* PŘÍMÉ SOUBOJE */}
        <section className="pb-10">
          <SectionHead title="Přímé souboje fondů" desc="Kdo vyhrává v populárních dvojicích a proč." href="/srovnani" hrefLabel="všechna srovnání" />
          <div className="grid sm:grid-cols-2 gap-3">
            {[
              { slug: 'cspx-vs-vuaa', href: '/srovnani?q=VUAA', label: 'CSPX vs VUAA', verdict: 'CSPX – stejný TER, výrazně větší fond a likvidita.' },
              { slug: 'vusa-vs-vuaa', href: '/srovnani?q=VUSA', label: 'VUSA vs VUAA', verdict: 'VUAA pro růst (akum), VUSA pro výplatu dividend.' },
              { slug: 'cspx-vs-spy5', href: '/srovnani?q=SPY5', label: 'CSPX vs SPY5', verdict: 'SPY5 levnější (TER 0,03 %), CSPX větší a akumulační.' },
              { slug: 'cspx-vs-vusa', href: '/srovnani?q=VUSA', label: 'CSPX vs VUSA', verdict: 'CSPX akumulační, VUSA distribuční – rozhoduje politika.' },
            ].map((d) => (
              <Link
                key={d.slug}
                href={`/srovnani/${d.slug}`}
        ...
        {/* JAK KOUPIT */}
        <section className="pb-10">
          <SectionHead title="Jak koupit" desc="Čtyři kroky od registrace k první akcii." href="/kde-koupit" hrefLabel="srovnání brokerů" />
          ...
          <div className="mt-3 flex flex-wrap gap-2.5">
            {['DEGIRO', 'XTB', 'Interactive Brokers'].map((b) => (
              <Link key={b} href="/kde-koupit" className="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:border-teal-300 hover:text-teal-700 transition-colors">{b}</Link>
            ))}
          </div>
        </section>
